...
var SnippetsList = class extends import_coc2.BasicList {
  ...
  async loadItems(context) {
    ...
    for (let snip of snippets) {
      ...
      let location = import_coc2.Location.create(import_coc2.Uri.file(snip.filepath).toString(), import_coc2.Range.create(pos, import_coc2.Position.create(snip.lnum, 1)));
...
var ProviderManager = class {
  constructor(channel, subscriptions) {
    ...
    subscriptions.push(import_coc3.Disposable.create(() => {
  ...
  async init() {
    ...
    await Promise.all(providers.map((provider) => {
      ...
      import_coc3.workspace.nvim.echoError(e);
  ...
  async provideCompletionItems(document, position, _token, context) {
    ...
    for (let snip of snippets) {
      ...
      if (snip.triggerKind == 1 /* LineBegin */ && contentBehind.trim().length)
        continue;
  ...
  async resolveCompletionItem(item) {
    ...
    if (provider) {
      let doc = import_coc3.workspace.getDocument(import_coc3.workspace.bufnr);
      let filetype = doc ? doc.filetype : void 0;
      let insertSnippet = item.data.snip.body;
      if (import_coc3.snippetManager && insertSnippet) {
        ...
        let resolved = await import_coc3.snippetManager.resolveSnippet(insertSnippet, item.data.ultisnip);
        if (typeof resolved !== "string") {
          import_coc3.window.showErrorMessage(`Please upgrade your coc.nvim to use coc-snippets`);
          return;
        }
...
var SnipmateProvider = class extends BaseProvider {
  constructor(channel, config, subscriptions) {
    ...
    this.fileItems = [];
    this.snippetFiles = [];
    import_coc4.workspace.onDidSaveTextDocument(async (doc) => {
      let uri = import_coc4.Uri.parse(doc.uri);
      if (uri.scheme != "file")
        return;
  ...
  async init() {
    ...
    for (let filetype of import_coc4.workspace.filetypes) {
      await this.loadByFiletype(filetype);
    }
    import_coc4.workspace.onDidOpenTextDocument(async (e) => {
      let doc = import_coc4.workspace.getDocument(e.bufnr);
...
var TextmateProvider = class extends BaseProvider {
  ...
  async init() {
    if (this.config.loadFromExtensions) {
      for (let extension of import_coc5.extensions.all) {
        await this.loadSnippetDefinition(extension);
      }
      import_coc5.extensions.onDidLoadExtension((extension) => {
        this.loadSnippetDefinition(extension).then((items) => {
          if (items == null ? void 0 : items.length) {
            items = items.filter((o) => o.languageIds.some((id) => import_coc5.workspace.languageIds.has(id)));
      ...
      import_coc5.extensions.onDidUnloadExtension((id) => {
    ...
    for (let languageId of import_coc5.workspace.languageIds) {
      await this.loadByLanguageId(languageId);
    }
    import_coc5.workspace.onDidOpenTextDocument((e) => {
      this.loadByLanguageId(e.languageId);
    }, null, this.subscriptions);
    if (this.config.projectSnippets) {
      import_coc5.workspace.workspaceFolders.forEach((folder) => {
        let fsPath = import_coc5.Uri.parse(folder.uri).fsPath;
        void this.loadFromWorkspace(fsPath);
      });
      import_coc5.workspace.onDidChangeWorkspaceFolders((e) => {
        e.removed.forEach((folder) => {
          let fsPath = import_coc5.Uri.parse(folder.uri).fsPath;
          this.loadedSnippets = this.loadedSnippets.filter((o) => {
            return !o.filepath.startsWith(fsPath + import_path3.default.sep);
          });
        });
  ...
  async loadFromWorkspace(fsPath) {
    let root = import_path3.default.join(fsPath, ".vscode");
  ...
  async getTriggerSnippets(document, position, autoTrigger) {
    ...
    for (let snip of snippets) {
      ...
      edits.push({
        prefix,
        range: import_coc5.Range.create(position.line, position.character - prefix.length, position.line, position.character),
        newText: snip.body,
  ...
  async loadDefinitionFromRoot(root) {
    root = import_coc5.workspace.expand(root);
    ...
    for (let file of files) {
      let filepath = import_path3.default.join(root, file);
      ...
      if (head == "endsnippet" && block == "snippet") {
        ...
        try {
          ...
          if (option.indexOf("r") !== -1) {
            ...
            regex2 = new RegExp(pattern.endsWith("$") ? pattern : pattern + "$");
            trigger = getRegexText(trigger);
            option = option + "i";
          }
          let snippet = {
            originRegex,
            context: parsedContext ? parsedContext : option.includes("e") ? ms[3] : void 0,
            filepath,
            filetype,
            prefix: trigger,
...
var UltiSnippetsProvider = class extends BaseProvider {
  constructor(channel, config, context) {
    ...
    import_coc6.workspace.onDidSaveTextDocument(async (doc) => {
      ...
      if (idx !== -1) {
        ...
        let filetype = filetypeFromBasename(import_path4.default.basename(filepath, ".snippets"));
        await this.loadSnippetsFromFile({ filetype, filepath, directory: import_path4.default.dirname(filepath) });
      }
    }, null, this.context.subscriptions);
  }
  get directories() {
    let dirs = this.config.directories || [];
    return dirs.map((dir) => import_coc6.workspace.expand(dir));
  }
  async init() {
    let { nvim, env } = import_coc6.workspace;
    ...
    await import_coc6.workspace.nvim.command(`exe 'pyxfile '.fnameescape('${filepath}')`);
  ...
  get allFiletypes() {
    let filetypes = Array.from(import_coc6.workspace.filetypes);
    let res = [];
...
` + code + "\n";
  ...
  async checkContext(context) {
    ...
    return await import_coc6.workspace.nvim.call(`pyxeval`, "True if context else False");
  }
  async executePyCodes(lines) {
    try {
      await import_coc6.workspace.nvim.command(`pyx ${addPythonTryCatch(lines.join("\n"))}`);
    } catch (e) {
...
` + e.stack;
  ...
  async getSubFolders() {
    ...
    let dirs = await import_coc6.workspace.nvim.eval('get(g:, "UltiSnipsSnippetDirectories", [])');
    for (let dir of dirs) {
  ...
  async executePythonCode(pythonCode) {
    try {
      ...
      await import_coc6.workspace.nvim.command(`exe 'pyxfile '.fnameescape('${tmpfile}')`);
    } catch (e) {
      this.error(`Error on execute python script ${e.stack}:`, pythonCode);
      import_coc6.window.showMessage(`Error on execute python script: ${e.message}`, "error");
    }
  }
};
...
function addPythonTryCatch(code) {
  if (!import_coc6.workspace.isVim)
    return code;
  let lines = [
...
async function getSnippetsDirectory(configuration) {
  ...
  if (snippetsDir) {
    snippetsDir = import_coc7.workspace.expand(snippetsDir);
    if (!import_path5.default.isAbsolute(snippetsDir)) {
      import_coc7.window.showMessage(`snippets.userSnippetsDirectory => ${snippetsDir} should be absolute path`, "warning");
      snippetsDir = null;
    }
  }
  if (!snippetsDir)
    snippetsDir = import_path5.default.join(import_path5.default.dirname(import_coc7.workspace.env.extensionRoot), "ultisnips");
  if (!import_fs6.default.existsSync(snippetsDir)) {
    await import_fs6.default.promises.mkdir(snippetsDir);
  }
  return snippetsDir;
}
async function insertSnippetEdit(edit) {
  ...
  await import_coc7.commands.executeCommand("editor.action.insertSnippet", import_coc7.TextEdit.replace(edit.range, edit.newText), option);
}
function enableSnippetsFiletype(subscriptions) {
  let { nvim } = import_coc7.workspace;
  import_coc7.workspace.documents.forEach((doc) => {
    if (doc.uri.endsWith(".snippets")) {
      doc.buffer.setOption("filetype", "snippets", true);
    }
  });
  import_coc7.workspace.onDidOpenTextDocument(async (document) => {
  ...
  const rtp = import_coc7.workspace.env.runtimepath;
  let paths = rtp.split(",");
...
async function snippetSelect() {
  let doc = await import_coc7.workspace.document;
  if (!doc)
    return;
  let { nvim } = import_coc7.workspace;
  let mode = await nvim.call("visualmode");
  if (["v", "V"].indexOf(mode) == -1) {
    import_coc7.window.showMessage(`visual mode ${mode} not supported`, "warning");
    return;
  }
  await nvim.command("normal! `<");
  let start = await import_coc7.window.getCursorPosition();
  await nvim.command("normal! `>");
  let end = await import_coc7.window.getCursorPosition();
  end = import_coc7.Position.create(end.line, end.character + 1);
  let range = import_coc7.Range.create(start, end);
  ...
  if (mode == "v") {
    ...
    text = lines.join("\n");
    range = import_coc7.Range.create(import_coc7.Position.create(start.line, indent.length), end);
    await doc.applyEdits([{ range, newText: "" }]);
  }
  await nvim.setVar("coc_selected_text", text);
  await import_coc7.window.moveTo(range.start);
}
async function activate(context) {
  var _a;
  let { subscriptions } = context;
  const { nvim } = import_coc7.workspace;
  const configuration = import_coc7.workspace.getConfiguration("snippets");
  ...
  const channel = import_coc7.window.createOutputChannel("snippets");
  ...
  if (!Array.isArray(excludes))
    excludes = [];
  excludes = excludes.map((p) => import_coc7.workspace.expand(p));
  ...
  if (configuration.get("autoTrigger", true)) {
    import_coc7.events.on("TextInsert", async (bufnr, info) => {
      let changedtick = info.changedtick;
      let doc = import_coc7.workspace.getDocument(bufnr);
      ...
      if (edits.length > 1) {
        ...
        import_coc7.window.showMessage("Multiple snippets found on auto trigger", "warning");
        await import_coc7.commands.executeCommand("workspace.showOutput", "snippets");
      }
  ...
  if (manager.hasProvider) {
    let disposable = import_coc7.languages.registerCompletionItemProvider("snippets", configuration.get("shortcut", "S"), null, manager, configuration.get("triggerCharacters", []), configuration.get("priority", 90));
  ...
  async function doExpand(bufnr) {
    ...
    if (edits.length == 1) {
      ...
      let idx = await import_coc7.window.showQuickpick(edits.map((e) => e.description || e.prefix), "choose snippet:");
      if (idx == -1)
        return;
  ...
  subscriptions.push(import_coc7.workspace.registerKeymap(["x"], "convert-snippet", async () => {
    let mode = await import_coc7.workspace.nvim.call("visualmode");
    if (!mode)
      return;
    let doc = await import_coc7.workspace.document;
    if (!doc)
      return;
    let range = await import_coc7.window.getSelectedRange(mode);
    let text = doc.textDocument.getText(range);
    if (text)
      await import_coc7.commands.executeCommand("snippets.editSnippets", text);
  }, { sync: false }));
  subscriptions.push(import_coc7.commands.registerCommand("snippets.editSnippets", async (text) => {
    let buf = await nvim.buffer;
    let doc = import_coc7.workspace.getDocument(buf.id);
    if (!doc) {
      import_coc7.window.showMessage("Document not found", "error");
      return;
    }
    ...
    if (!import_fs6.default.existsSync(file)) {
      await import_util8.default.promisify(import_fs6.default.writeFile)(file, documentation, "utf8");
    }
    let uri = import_coc7.Uri.file(file).toString();
    await import_coc7.workspace.jumpTo(uri, null, configuration.get("editSnippetsCommand"));
    if (text) {
      await nvim.command("normal! G");
      await nvim.command("normal! 2o");
      let position = await import_coc7.window.getCursorPosition();
      ...
      let edit = import_coc7.TextEdit.insert(position, snippet);
      await import_coc7.commands.executeCommand("editor.action.insertSnippet", edit, false);
    }
  }));
  subscriptions.push(import_coc7.commands.registerCommand("snippets.openOutput", () => {
    void import_coc7.window.showOutputChannel("snippets", false);
  }));
  subscriptions.push(import_coc7.commands.registerCommand("snippets.openSnippetFiles", async () => {
    let buf = await nvim.buffer;
    let doc = import_coc7.workspace.getDocument(buf.id);
    if (!doc) {
      import_coc7.window.showMessage("Document not found", "error");
    ...
    if (!files.length) {
      import_coc7.window.showMessage("No related snippet file found", "warning");
      return;
    }
    let idx = await import_coc7.window.showQuickpick(files, "choose snippet file:");
    if (idx == -1)
      return;
    let uri = import_coc7.Uri.file(files[idx]).toString();
    await import_coc7.workspace.jumpTo(uri, null, configuration.get("editSnippetsCommand"));
  }));
  subscriptions.push(import_coc7.workspace.registerKeymap(["i"], "snippets-expand", async () => {
    let bufnr = await nvim.eval('bufnr("%")');
    let expanded = await doExpand(bufnr);
    if (!expanded)
      await fallback();
  }, { silent: true, sync: true, cancel: true }));
  subscriptions.push(import_coc7.workspace.registerKeymap(["i"], "snippets-expand-jump", async () => {
    let bufnr = await nvim.eval('bufnr("%")');
    let expanded = await doExpand(bufnr);
    if (!expanded) {
      let session = import_coc7.snippetManager.getSession(bufnr);
      if (session && session.isActive) {
        await nvim.call("coc#_cancel", []);
        await import_coc7.snippetManager.nextPlaceholder();
        return;
      }
      await fallback();
    }
  }, { silent: true, sync: true, cancel: true }));
  subscriptions.push(import_coc7.workspace.registerKeymap(["v"], "snippets-select", snippetSelect, { silent: true, sync: false, cancel: true }));
  let languageProvider = new LanguageProvider(channel, trace);
  subscriptions.push(import_coc7.languages.registerCompletionItemProvider("snippets-source", configuration.get("shortcut", "S"), ["snippets"], languageProvider, ["$"], configuration.get("priority", 90)));
  subscriptions.push(channel);
  subscriptions.push(import_coc7.listManager.registerList(new SnippetsList(import_coc7.workspace.nvim, manager)));
  return {
    expandable: async () => {
      let bufnr = await nvim.eval('bufnr("%")');
      let edits = await manager.getTriggerSnippets(bufnr);
